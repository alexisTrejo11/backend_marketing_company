...
public interface CampaignMetricJpaRepository extends JpaRepository<CampaignMetricEntity, Long> {
    ...
    @Query("SELECT m FROM CampaignMetricEntity m WHERE m.deletedAt IS NULL "
            + "AND m.campaign.id = :campaignId AND m.isAutomated = :isAutomated")
    Page<CampaignMetricEntity> findByCampaignIdAndIsAutomated(@Param("campaignId") Long campaignId, @Param("isAutomated") Boolean isAutomated, Pageable pageable);

    @Query("SELECT m FROM CampaignMetricEntity m WHERE m.deletedAt IS NULL "
            + "AND m.campaign.id = :campaignId AND m.isTargetAchieved = :isTargetAchieved")
    Page<CampaignMetricEntity> findByCampaignIdAndIsTargetAchieved(@Param("campaignId") Long campaignId, @Param("isTargetAchieved") Boolean isTargetAchieved, Pageable pageable);
}
